...
An autonomous music metadata enrichment pipeline targeting 10M–30M tracks. Discovers tracks via Spotify, Last.fm, YouTube Music, Discogs and an artist collaboration graph, enriches them with quality scores, YouTube video IDs and supplementary metadata, and stores everything in MongoDB. All workers share a single Docker image; the `WORKER_TYPE` env var selects which worker runs.
...
|---|---|---|---|
| `seeder` | — | playlist_queue | Runs once, exits |
...
| `musicbrainz_worker` | `musicbrainz_enriched` | MBID, composers, release data — **1 req/s, single replica only** |
...
discogs_worker  ┘         (29K+)            (Spotify search, score ≥ 0.8)
                                                      ↓
...
| `candidate_match_worker` | track_candidates | rapidfuzz 60/40 title/artist scoring, threshold 0.8 |
...
**Priority formula**: `0.4 × (popularity/100) + 0.3 × (log(followers)/14)` — see `compute_artist_priority()` in `app/models/artist_graph.py`.
...
| `app/models/track.py` | `TrackDocument`, `TrackStatus`, `YoutubeData`, `ArtistRef`, `AlbumRef` |
...
| `app/services/spotify.py` | Token refresh, rate limiting, tenacity retry, all Spotify endpoints |
...
| `app/utils/scoring.py` | Quality score formula |
